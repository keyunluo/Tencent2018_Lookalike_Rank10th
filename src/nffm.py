...
class Model(object):
    def __init__(self, hparams):
        ...
        self.bias = tf.Variable(tf.truncated_normal(
            shape=[1], mean=0.0, stddev=0.0001), name='bias')
        self.use_dropout = tf.placeholder(tf.bool)
        initializer = tf.random_uniform_initializer(-0.1, 0.1)
        self.feature_all_length = len(
            hparams.single_features)+len(hparams.mutil_features)
        ...
        for s in hparams.single_features:
            ...
            self.emb_v1[s] = tf.Variable(tf.truncated_normal(
                shape=[len(hparams.dict[s])+2, 1], mean=0.0, stddev=0.0001), name='emb_v1_'+s)
            if s in self.f1:
                self.emb_v2[s] = tf.Variable(tf.truncated_normal(shape=[len(
                    hparams.dict[s])+2, len(self.f2), hparams.k], mean=0.0, stddev=0.0001), name='emb_v2_'+s)
            elif s in self.f2:
                self.emb_v2[s] = tf.Variable(tf.truncated_normal(shape=[len(
                    hparams.dict[s])+2, len(self.f1), hparams.k], mean=0.0, stddev=0.0001), name='emb_v2_'+s)

        for s in hparams.mutil_features:
            self.mulit_ids[s] = tf.placeholder(
                shape=(None, None), dtype=tf.int32)
            self.length[s] = tf.placeholder(shape=(None,), dtype=tf.int32)
            self.mulit_mask[s] = tf.sequence_mask(
                self.length[s], 100, dtype=tf.float32)
            self.emb_v1[s] = tf.get_variable(
                shape=[len(hparams.dict[s])+2, 1], initializer=initializer, name='emb_v1_'+s)
            if s in self.f1:
                self.emb_v2[s] = tf.Variable(tf.truncated_normal(shape=[len(
                    hparams.dict[s])+2, len(self.f2), hparams.k], mean=0.0, stddev=0.0001), name='emb_v2_'+s)
            elif s in self.f2:
    ...
    def build_graph(self, hparams):
        ...
        for s in hparams.mutil_features:
            emb_inp_v1[s] = tf.reduce_sum(tf.gather(self.emb_v1[s], self.mulit_ids[s]) *
                                          self.mulit_mask[s][:, :, None], axis=1)/tf.cast(self.length[s], tf.float32)[:, None]
        ...
        for s in hparams.mutil_features:
            emb_inp_v2[s] = tf.reduce_sum(tf.gather(self.emb_v2[s], self.mulit_ids[s])*self.mulit_mask[s]
                                          [:, :, None, None], axis=1) / tf.cast(self.length[s], tf.float32)[:, None, None]
        ...
        emb_rep_v = x[0]*x[1]
        emb_rep_v = tf.reshape(
            emb_rep_v, [-1, len(self.f1)*len(self.f2)*hparams.k])
        emb_rep = emb_rep_v
        temp = []
        temp.append(emb_rep)
        for s in hparams.num_features:
        ...
        input_size = len(self.f1)*len(self.f2)*hparams.k + \
            len(hparams.num_features)
        glorot = np.sqrt(2.0 / (input_size + hparams.hidden_size[0]))

        W_1 = tf.get_variable("W_1", [emb_rep.get_shape(
        )[-1], hparams.hidden_size[0]], initializer=initializer)
        b_1 = tf.get_variable(
            "b_1", [1, hparams.hidden_size[0]], initializer=initializer)

        hidden_outputs_1 = tf.tensordot(emb_rep, W_1, [[-1], [0]])
        hidden_outputs_1 = self.batch_norm_layer(
            hidden_outputs_1, self.use_dropout, 'train_1')
        hidden_outputs_1 = tf.nn.relu(hidden_outputs_1)

        W_2 = tf.get_variable(
            "W_2", [hparams.hidden_size[0], hparams.hidden_size[1]], initializer=initializer)
        b_2 = tf.get_variable(
            "b_2", [1, hparams.hidden_size[1]], initializer=initializer)

        hidden_outputs_2 = tf.tensordot(hidden_outputs_1, W_2, [[-1], [0]])
        hidden_outputs_2 = self.batch_norm_layer(
            hidden_outputs_2, self.use_dropout, 'train_2')
        hidden_outputs_2 = tf.nn.relu(hidden_outputs_2)

        glorot = np.sqrt(2.0 / (hparams.hidden_size[1] + 1))
        W_3 = tf.get_variable(
            "W_3", [hparams.hidden_size[1], 1], initializer=initializer)
    ...
    def optimizer(self, hparams):
        ...
        if hparams.optimizer == "sgd":
            ...
            opt = tf.train.AdamOptimizer(
                self.lrate, beta1=0.9, beta2=0.999, epsilon=1e-8)
        elif hparams.optimizer == "ada":
            opt = tf.train.AdagradOptimizer(
                learning_rate=self.lrate, initial_accumulator_value=1e-8)
        params = tf.trainable_variables()

        gradients = tf.gradients(
            self.cost, params, colocate_gradients_with_ops=True)
...
def train(hparams):
    ...
    dev_iterator = TextIterator(
        hparams, mode="dev", batch_size=hparams.evl_batch_size)
    test_iterator = TextIterator(
        hparams, mode="test", batch_size=hparams.evl_batch_size)
    model = Model(hparams)
    config_proto = tf.ConfigProto(
        log_device_placement=0, allow_soft_placement=0)
    ...
    if hparams.mode != 'train':
        model.saver_ffm.restore(sess, os.path.join(
            hparams.path, 'model_'+str(hparams.sub_name)))
    else:
        # if os.path.exists(os.path.join(hparams.path, 'model_'+str(hparams.idx))):
        ...
        while True:
            ...
            if global_step % hparams.num_display_steps == 0 or global_step >= hparams.epoch:
                ...
                info["train_ppl"] = train_loss / \
                    hparams.num_display_steps
                info["avg_grad_norm"] = train_norm / \
                    hparams.num_display_steps
                train_loss = 0
                train_norm = 0
                ...
                if global_step % hparams.num_eval_steps == 0 or global_step >= hparams.epoch:
                    ...
                    loss_ = np.mean(auc)
                    if best_loss <= loss_:
                        model.saver_ffm.save(sess, os.path.join(
                            hparams.path, 'model_'+str(hparams.sub_name)))
                        best_loss = loss_
                        T = (time.time()-start_time)
                        start_time = time.time()
                        utils.print_out(
                            "# Epcho-time %.2fs Eval AUC %.6f. Best AUC %.6f." % (T, loss_, best_loss))
                    else:
                        utils.print_out(
                            "# Epcho-time %.2fs Eval AUC %.6f. Best AUC %.6f." % (T, loss_, best_loss))
                        model.saver_ffm.restore(sess, os.path.join(
    ...
    df.to_csv('submission_dev_' +
              str(hparams.sub_name)+'.csv', index=False)
